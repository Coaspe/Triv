...
function EditableField({ value, field, modelId, className = "" }: EditableFieldProps) {
...
function EditableLink({ value, field, modelId, icon }: EditableLinkProps) {
  const [isEditing, setIsEditing] = useState(false);
  const [editValue, setEditValue] = useState(value || "");
  const [hasChanges, setHasChanges] = useState(false);
...
function ImageManager({ images, modelId, singedImageUrls }: { images: string[]; modelId: string; singedImageUrls: { [key: string]: string } }) {
  const [isEditing, setIsEditing] = useState(false);
  ...
  return (
    <div className="relative group">
      ...
      <div className="relative aspect-[3/4] overflow-hidden shadow-md">
        <Image src={singedImageUrls[images[0]]} alt="Profile" fill style={{ objectFit: "cover" }} priority />
        {/* 편집 버튼 */}
        <button
      ...
      {/* 이미지 관리 모달 */}
      {isEditing && <ImageEditModal images={images} modelId={modelId} singedImageUrls={singedImageUrls} onClose={() => setIsEditing(false)} />}
    </div>
  );
}

function ImageEditModal({ images, modelId, singedImageUrls, onClose }: { images: string[]; modelId: string; singedImageUrls: { [key: string]: string }; onClose: () => void }) {
                      ...
                      >
                        <div className={`relative aspect-[3/4] ${snapshot.isDragging ? "z-50" : ""}`}>
                          <div className="absolute inset-0 bg-white rounded overflow-hidden">
                            <Image src={singedImageUrls[image]} alt={`Image ${index + 1}`} fill className="object-cover" />
                            <div
...
export default function ModelDetailClient({ modelData }: { modelData: ModelDetails }) {
    ...
    <div className="max-w-[1200px] mx-auto px-6 pb-6 text-black rounded-lg">
      ...
      <div className="flex flex-col md:flex-row gap-12 mb-16">
        ...
        <div className="md:w-1/2">
          <ImageManager singedImageUrls={modelData.singedImageUrls || {}} images={modelData.images || []} modelId={modelData.id} />
        </div>

        {/* 모델 정보 */}
        <div className="md:w-1/2 ml-5">
          <h1 className="text-4xl font-bold mb-6">
            <EditableField value={modelData.displayName} field="displayName" modelId={modelData.id} className="text-4xl font-bold" />
          </h1>

          {/* 소셜 미디어 링크 */}
          <div className="flex gap-4 mb-6">
            <EditableLink value={modelData.instagram} field="instagram" modelId={modelData.id} icon={<FaInstagram />} />
            <EditableLink value={modelData.tiktok} field="tiktok" modelId={modelData.id} icon={<FaTiktok />} />
            <EditableLink value={modelData.youtube} field="youtube" modelId={modelData.id} icon={<FaYoutube />} />
          </div>
            ...
            modelData.singedImageUrls ? (
              <div key={index} className="flex flex-col items-center cursor-pointer relative aspect-[3/4] rounded-lg overflow-hidden shadow-md hover:scale-105 transition-transform duration-300">
                <div className="relative w-full aspect-[3/4]" onClick={() => handleImageClick(index + 1)}>
                  <Image src={modelData.singedImageUrls[image]} alt={`${modelData.name} ${index + 2}`} fill className="object-cover" sizes="(max-width: 768px) 50vw, 25vw" />
                </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 z-50 flex items-center justify-center">
          ...
          {modelData.images && modelData.images.length > 1 && (
            <div className="relative h-[80vh] w-[800px] max-w-[90vw]">
              <Image
                src={modelData.singedImageUrls && modelData.images ? modelData.singedImageUrls[modelData.images[selectedImageIndex]] : ""}
